...
def download(
    ...
    if not os.path.exists(data_file) and online is False:
    ...
    # Pull file from github but don't save
        ...
        logger.info("Using github version of data")
        return pd.read_csv(
            io.StringIO(
...
def process_data(df, save=True, scale=True):
    ...
    Input:
        df: raw data frame. May contain features only or  features and labels
    ...
    if df_z_scaled.shape[0] != 1:
        for num in num_features:
            df_z_scaled[num] = (
                df_z_scaled[num] - df_z_scaled[num].mean())\
                / df_z_scaled[num].std()
    ...
    if not os.path.exists(
            './clean_data.csv') and save and df_z_scaled.shape[1] == 15:
...
def slices(feature, df, classes=None, property=None):
    ...
    Classes: a list of potential values for the categorical feature, e.g.
        ...
        "Self-emp-not-inc", "?", "State-gov", "Without-pay"] for
        workclass feature

    """
    # Show mean possible numeric values of all potential workclass categoricals
    """
                                age          fnlgt   capital-loss  hrs-per-week
    workclass
    ?                 40.96  188516.338235      60.760349       31.919390
    Federal-gov       42.59  185221.243750    112.268750       41.379167
    Local-gov         41.751  188639.712852    109.854276       40.982800
    """
...
def split(df, stratify_by=None):
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=0.25, stratify=X[stratify_by] if stratify_by else None,
        random_state=42)
...
def train_models(df, stratify_by=None):
    ...
    # Save best models
    joblib.dump(cv_rfc.best_estimator_, './models/rfc_model.pkl', compress=5)
    ...
    if feature is not None:
        try:
            ...
            for (subcategory, hot) in both:
                # Get subset of data rows containing sub-feature slice, e.g.
                # "Male" in "Sex"
                ...
                # Predict on features data set. ValueError if there are no
                # predicted samples
                try:
                ...
                # Save classification report and ROC score for given slice to
                # file
                    ...
                    try:
                        report = str(
                            classification_report(
                                label_subset, preds))
                        print(
                            f"Class report for {feature}:\
                            {subcategory} is ", file=f)
                        print(report, file=f)
                    except ValueError:
                        pass
                    try:
                        ...
                        print(
                            f"Roc AUC for {feature}: {subcategory}\
                            is: {auc}", file=f)
                    except ValueError:
                        pass
        except AssertionError:
...
if __name__ == '__main__':
    # Load data, perform eda, encode categorical features, and engineer
    # features
    data_frame = download()
